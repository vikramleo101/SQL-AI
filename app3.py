...
def execute_query(query, df_name="df"):
    ...
    try:
        if st.session_state.db_conn:  # SQLite database
            ...
            # Use a dictionary for local variables
            return sqldf(query, {'df': st.session_state.df})
...
3. For CSV/Excel data, use 'df' as the table name
...
with tab2:
    ...
    data_source = st.radio("Data Source", 
                          ["Original Data", "Last Query Result"],
                          horizontal=True)
    
    if data_source == "Original Data":
        df = st.session_state.df
    elif st.session_state.last_analysis and st.session_state.last_analysis["result_df"] is not None:
